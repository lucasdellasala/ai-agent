...
export class Context {
  static get<T extends ContextKey>(key: T): IContext[T] | undefined {
    ...
    return store?.get(key) as IContext[T] | undefined;
  }

  static getFullContext(): IContext {
    const store = contextStorage.getStore();
    return store ? (Object.fromEntries(store) as IContext) : {};
  }

  static update<T extends ContextKey>(key: T, value: IContext[T]): void {
    const store = contextStorage.getStore();
    if (store) {
      store.set(key, value);
    }
